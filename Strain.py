    ...
    for e in range(1, epochs + 1):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            ...
            if iter_ % 100 == 0:
                ...
                print('SDbranch Train (epoch {}/{}) [{}/{} ({:.0f}%)]\tLoss: {:.6f}\tAccuracy: {}'.format(e, epochs, batch_idx,
                                                                                                 len(train_loader),
                                                                                                 100. * batch_idx / len(
                                                                                                     train_loader),
                                                                                                 loss.item(),
        ...
        if os.path.exists('./SDbranch/') is False:
            os.mkdir('./SDbranch/')

        if e % save_epoch == 0:
            # acc = test(net, test_ids, all=False, stride=min(WINDOW_SIZE))
            torch.save(net.state_dict(), './SDbranch/sd_{}'.format(e))
    torch.save(net.state_dict(), './SDbranch/sd_final')
